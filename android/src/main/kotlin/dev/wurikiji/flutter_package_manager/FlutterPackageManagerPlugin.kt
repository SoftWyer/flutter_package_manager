...
class FlutterPackageManagerPlugin: FlutterPlugin, MethodCallHandler {
  ...
  override fun onAttachedToEngine(binding: FlutterPluginBinding) {
      val channel = MethodChannel(binding.getBinaryMessenger(), METHOD_CHANNEL, JSONMethodCodec.INSTANCE)
      channel.setMethodCallHandler(this)
      sContext = binding.getApplicationContext()
      Log.i(TAG, "onAttachedToEngine with ${sContext?.getApplicationInfo()}")
  }
  ...
  private fun getPackageInfo(packageName: String) : java.util.HashMap<String, Any?>? {
    ...
    try {
      ...
      val appIcon: Drawable = (sContext!!.packageManager.getApplicationIcon(appInfo.packageName) ?: sContext!!.getDrawable(R.drawable.ic_launcher)) as Drawable
